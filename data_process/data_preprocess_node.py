...
def process_mol(input_dir, output_dir):
    """
    处理指定的分子文件夹：每个 .xyz → 两个 .npy（atom_numbers & coordinates）
    :param base_dir: 基目录路径
    :param folder_name: 分子文件夹名称
    :param output_dir: 输出目录路径
    """
    xyz_files = [f for f in os.listdir(input_dir) if f.endswith('.xyz')]
    xyz_reader = XYZReader(input_dir)
    ...
    for file_name, (anum, coord) in zip(xyz_files, zip(atom_numbers, coordinates)):
        ...
        np.save(os.path.join(output_dir, f"{base}_atom_numbers.npy"), np.array(anum))
        np.save(os.path.join(output_dir, f"{base}_coordinates.npy"), np.array(coord))
    print(f"Processed {len(xyz_files)} files, saved to {output_dir}")
